...
impl Node {
    ...
    pub async fn run_rx_loop(&mut self) -> Result<(), NodeError> {
        ...
        let (control_tx, mut control_rx) = if let Some(rx) = self.control_rx.take() {
            // Use the pre-set sender if available, otherwise we still need
            // a dummy for the control server to bind to (though it won't work
            // perfectly for the socket until we fix that too).
            // Actually, we should use self.control_tx if it was set.
            let tx = self.control_tx.take().unwrap_or_else(|| {
        ...
        loop {
            tokio::select! {
                ...
                Some((request, response_tx)) = control_rx.recv() => {
                    if request.command.starts_with("show_") {
