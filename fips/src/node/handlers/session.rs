...
impl Node {
                ...
                match dst_port {
                    FSP_PORT_IPV6_SHIM => {
                        ...
                        match crate::upper::ipv6_shim::decompress_ipv6(service_payload, src_ipv6, dst_ipv6) {
                            Some(mut packet) => {
                                // Check if this is an ICMPv6 Echo Reply for an internal ping
                                if packet.len() >= 48 && packet[6] == 58 && packet[40] == 129 {
                                    let seq = u16::from_be_bytes([packet[46], packet[47]]);
                                    if let Some((start, tx)) = self.pending_pings.remove(&(*src_addr, seq)) {
                                        let rtt = start.elapsed();
                                        let response = crate::control::protocol::Response::ok(serde_json::json!({
                                            "src_addr": src_addr.to_string(),
                                            "seq": seq,
                                            "rtt_ms": rtt.as_secs_f64() * 1000.0,
                                        }));
                                        let _ = tx.send(response);
                                        // Still deliver to TUN if active
                                    }
                                }
                                ...
                                #[cfg(feature = "tun-support")]
                                if let Some(tun_tx) = &self.tun_tx_handle() {
    ...
    pub async fn handle_tun_outbound(&mut self, ipv6_packet: Vec<u8>) {
    ...
    pub(in crate::node) fn send_icmpv6_packet_too_big(&mut self, original_packet: &[u8], mtu: u32) {
        #[cfg(feature = "tun-support")]
        {
            ...
            if let Some(response) = build_packet_too_big(original_packet, mtu, dest_addr)
                && let Some(tun_tx) = &self.tun_tx_handle()
            {
                debug!(
