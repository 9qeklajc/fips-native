...
impl Node {
    ...
    /// Check for timed-out internal pings and notify callers.
    pub(in crate::node) fn check_ping_timeouts(&mut self) {
        if self.pending_pings.is_empty() {
            return;
        }

        let now = std::time::Instant::now();
        let timeout = std::time::Duration::from_secs(5); // 5s timeout

        let timed_out: Vec<_> = self.pending_pings.iter()
            .filter(|(_, (start, _))| now.duration_since(*start) > timeout)
            .map(|(k, _)| *k)
            .collect();

        for key in timed_out {
            if let Some((_, tx)) = self.pending_pings.remove(&key) {
                let _ = tx.send(crate::control::protocol::Response::error("Ping timeout"));
            }
        }
    }
}
