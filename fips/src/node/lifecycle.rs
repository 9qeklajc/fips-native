...
impl Node {
    ...
    ///
    /// This is used on Android where the TUN device is created by VpnService.
    #[cfg(feature = "tun-support")]
    pub async fn start_with_tun_fd(&mut self, fd: std::os::unix::io::RawFd) -> Result<(), NodeError> {
        self.start_internal(Some(fd)).await
    }

    #[cfg(not(feature = "tun-support"))]
    pub async fn start_with_tun_fd(&mut self, _fd: std::os::unix::io::RawFd) -> Result<(), NodeError> {
        self.start_internal(None).await
    }

    /// Internal start implementation.
    async fn start_internal(&mut self, tun_fd: Option<std::os::unix::io::RawFd>) -> Result<(), NodeError> {
