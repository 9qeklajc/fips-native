...
///
/// Arguments:
/// - `src_addr`: Our FIPS IPv6 address
/// - `dest_addr`: Target FIPS IPv6 address
/// - `id`: Identifier to match requests/replies
/// - `seq`: Sequence number
...
pub fn build_echo_request(
    ...
    let icmpv6_len = ICMPV6_HEADER_LEN + payload.len();
    let total_len = IPV6_HEADER_LEN + icmpv6_len;
    ...
    packet[24..40].copy_from_slice(&dest_addr.octets());

    // === ICMPv6 Header ===
    let icmp_start = IPV6_HEADER_LEN;
    packet[icmp_start] = Icmpv6Type::EchoRequest as u8;
    ...
    // === ICMPv6 Body ===
    packet[icmp_start + ICMPV6_HEADER_LEN..].copy_from_slice(payload);

    // Calculate checksum
    let checksum = icmpv6_checksum(&packet[icmp_start..], &src_addr, &dest_addr);
    packet[icmp_start + 2..icmp_start + 4].copy_from_slice(&checksum.to_be_bytes());
