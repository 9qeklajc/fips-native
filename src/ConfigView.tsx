          ...
          <section className="space-y-4">
            ...
            <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6">
              ...
              <p className="text-xs text-neutral-500 mb-4">
                Send an ICMPv6 Echo Request to a node in the mesh via its NPUB or Node Address.
              </p>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-black border border-neutral-800 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500 transition-colors"
                />
                <button
                  onClick={handlePing}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <label className="text-sm text-neutral-400">Persistent Identity</label>
                  <input
                    type="checkbox"
                    checked={config.node.persistent}
                    onChange={(e) => setConfig({ ...config, node: { ...config.node, persistent: e.target.checked } })}
                    className="w-4 h-4"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] text-neutral-500 uppercase font-bold">NSEC (Optional)</label>
                  <input
                    type="password"
                    value={config.node.nsec || ""}
                    onChange={(e) => setConfig({ ...config, node: { ...config.node, nsec: e.target.value || null } })}
                    placeholder="Auto-generated if empty"
                    className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                  />
                </div>
              </div>
              ...
              <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={(e) => setConfig({ ...config, tun: { ...config.tun, enabled: e.target.checked } })}
                    className="w-4 h-4"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) => setConfig({ ...config, tun: { ...config.tun, name: e.target.value } })}
                      className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] text-neutral-500 uppercase font-bold">MTU</label>
                    <input
                      type="number"
                      value={config.tun.mtu}
                      onChange={(e) => setConfig({ ...config, tun: { ...config.tun, mtu: parseInt(e.target.value) } })}
                      className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                    />
                  </div>
                </div>
            ...
            <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h5 className="text-xs font-bold text-white uppercase tracking-wider">UDP Transport</h5>
                    <input
                      type="checkbox"
                      checked={config.transports.udp_enabled}
                      onChange={(e) => setConfig({ ...config, transports: { ...config.transports, udp_enabled: e.target.checked } })}
                      className="w-3 h-3"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] text-neutral-500 uppercase font-bold">Bind Address</label>
                    <input
                      type="text"
                      value={config.transports.udp_bind_addr}
                      onChange={(e) => setConfig({ ...config, transports: { ...config.transports, udp_bind_addr: e.target.value } })}
                      className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h5 className="text-xs font-bold text-white uppercase tracking-wider">TCP Transport</h5>
                    <input
                      type="checkbox"
                      checked={config.transports.tcp_enabled}
                      onChange={(e) => setConfig({ ...config, transports: { ...config.transports, tcp_enabled: e.target.checked } })}
                      className="w-3 h-3"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) => setConfig({ ...config, transports: { ...config.transports, tcp_bind_addr: e.target.value } })}
                      className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3 col-span-1 md:col-span-2 pt-4 border-t border-neutral-800">
                  <div className="flex items-center justify-between">
                    <h5 className="text-xs font-bold text-white uppercase tracking-wider">Ethernet Transport</h5>
                    <input
                      type="checkbox"
                      checked={config.transports.ethernet_enabled}
                      onChange={(e) => setConfig({ ...config, transports: { ...config.transports, ethernet_enabled: e.target.checked } })}
                      className="w-3 h-3"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] text-neutral-500 uppercase font-bold">Interface</label>
                    <input
                      type="text"
                      value={config.transports.ethernet_interface}
                      onChange={(e) => setConfig({ ...config, transports: { ...config.transports, ethernet_interface: e.target.value } })}
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {config.peers.map((peer, idx) => (
                <div key={idx} className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-5 space-y-4 relative group">
                  <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] text-neutral-500 uppercase font-bold">Alias</label>
                      <input
                        type="text"
                        value={peer.alias}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-neutral-500 uppercase font-bold">NPUB</label>
                      <input
                        type="text"
                        value={peer.npub}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-neutral-500 uppercase font-bold">Address</label>
                      <input
                        type="text"
                        value={peer.address}
                        onChange={(e) => updatePeer(idx, "address", e.target.value)}
                        placeholder="host:port"
                        className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] text-neutral-500 uppercase font-bold">Transport</label>
                      <select
                        value={peer.transport}
                        onChange={(e) => updatePeer(idx, "transport", e.target.value)}
                        className="w-full bg-black border border-neutral-800 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-blue-500"
                      >
              ...
              {config.peers.length === 0 && (
                <div className="text-center py-8 border-2 border-dashed border-neutral-900 rounded-2xl">
                  <p className="text-neutral-600 text-sm">No static peers configured.</p>
                </div>
              )}
            </div>
          </section>
