...
function buildLayout(tree: TreeData, directPeers: DirectPeer[]): { nodes: LayoutNode[]; edges: LayoutEdge[] } {
  ...
  const minDepth = depthLevels[0] ?? 0
...
export function TreeGraph({ tree, peers: directPeers }: { tree: TreeData; peers: DirectPeer[] }) {
  ...
  return (
    <div className="overflow-hidden">
      ...
      <svg
        ref={svgRef}
        className="w-full h-[400px] sm:h-[600px] bg-neutral-950 rounded-md border border-neutral-900 cursor-grab active:cursor-grabbing"
      >
        <g ref={gRef}>
          {/* edges */}
          {edges.map(e => {
            ...
            // Color the edge based on SRTT latency
            let strokeColor = e.isTreeEdge ? (hasMetrics ? '#6b7280' : '#4b5563') : '#854d0e'
            if (e.srtt_ms != null) {
              if (e.srtt_ms < 50) strokeColor = '#22c55e' // Green: low latency
              else if (e.srtt_ms < 150) strokeColor = '#eab308' // Yellow: moderate latency
              else strokeColor = '#ef4444' // Red: high latency
            }
